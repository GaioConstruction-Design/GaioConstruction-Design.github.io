...
(function () {
  ...
  const I18N = {
    el: {
      ...
      'services.h2': 'Tομείς εξειδικευμένης παροχής',
      'services.intro':
      ...
      'service.5.desc':
        'Εξειδικευμένες Γεωτεχνικές Μελέτες θεμελίωσης κατασκευών, αντιστήριξης εδάφους, ευστάθειας πρανών και αποκατάστασης κατολισθήσεων, βραχοπτώσεων, στατικών μελετών διαστασιολόγησης και όπλισης έργων αντιστήριξης, καθώς και μελέτες υδραυλικών δικτύων, με χρήση κατάλληλων λογισμικών.',
      ...
      'project1.body.p1':
        'Παλαιός πετρόχτιστος τοίχος αντιστήριξης παρουσίαζε σημάδια παραμόρφωσης. Κατά τη διάρκεια εργασιών οδοποιίας έμπροσθεν του τοίχου υπήρχε αυξημένος κίνδυνος κατάρρευσης.',
      'project1.body.p2':
    ...
    en: {
      ...
      'hero.lead':
        'Specialised geotechnical, structural and hydraulic studies for infrastructure projects — with 24+ years of experience in soil–structure interaction.',
  ...
  function buildTweaksPanel() {
    ...
    panel.querySelectorAll('[data-accent]').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        applyTweaks(tweaks);
        saveTweaks(tweaks);
        panel.querySelectorAll('[data-accent]').forEach((b) => b.setAttribute('aria-pressed', b === btn ? 'true' : 'false'));
      });
    });
    panel.querySelectorAll('[data-density]').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        applyTweaks(tweaks);
        saveTweaks(tweaks);
        panel.querySelectorAll('[data-density]').forEach((b) => b.setAttribute('aria-pressed', b === btn ? 'true' : 'false'));
      });
    });
    panel.querySelectorAll('[data-theme]').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        applyTweaks(tweaks);
        saveTweaks(tweaks);
        panel.querySelectorAll('[data-theme]').forEach((b) => b.setAttribute('aria-pressed', b === btn ? 'true' : 'false'));
      });
    });
  ...
  function initContactForm() {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        const err = lang === 'el' ? 'Σφάλμα σύνδεσης. Παρακαλώ δοκιμάστε ξανά.' : 'Connection error. Please try again.';
        if (slot) {
          slot.textContent = err;
          slot.hidden = false;
          slot.style.color = '#c9a227';
